...
const debug = require('debug')('appmetrics-codewind');
...
function attach(options) {
  ...
  patchHttpModule(save.http, require('http'));
  patchHttpModule(save.https, require('https'));

  function patchHttpModule(save, http) {
    // Patch the constructor as well as createServer.
    save.Server = http.Server;
    http.Server = function(...args) {
      const server = save.Server.apply(this, args);
    ...
    http.createServer = function(...args) {
      const server = save.createServer.apply(this, args);
      const app = args.find(arg => (typeof arg.use) === 'function');
      options.app = app;
      options.server = server;
      monitor(options);
      return server;
...
function monitor(options) {
  ...
  if (server && app) {
    const ourApp = createOurApp(options);
    app.use(ourApp);
  } else {
    debug('patch existing request listeners');
    server.listeners('request').forEach(listener => patchListener(listener));
    ...
    server.on('newListener', function(eventName, listener) {
      ...
      process.nextTick(function() { patchListener(listener); });
    });
  }

  function patchListener(listener) {
...
function createOurApp(options) {
  const app = express();

  const monitoring = startAppmetricsMonitor(options);
...
function startAppmetricsMonitor(options) {
  ...
  const appmetrics = options.appmetrics || require('appmetrics');
